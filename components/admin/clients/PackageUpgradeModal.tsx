...
export function PackageUpgradeModal({ clientId, onSuccess, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true" aria-labelledby="upgrade-modal-title">
      <div ref={containerRef} className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-5">
        <div className="flex items-center justify-between">
          <h2 id="upgrade-modal-title" className="text-lg font-bold text-navy">Initiate Package Upgrade</h2>
          <button onClick={onClose} className="p-1 text-body hover:text-navy transition-colors" aria-label="Close">
            <X className="h-5 w-5" aria-hidden="true" />
          </button>
        </div>

        {fetchError && <p className="text-sm text-red-600">Failed to load packages.</p>}

        {!fetchError && (
          <div className="space-y-4">
            <div>
              <label className="ec-label" htmlFor="upgrade-package">New package</label>
              <select id="upgrade-package" className="ec-select admin-select" value={selectedId} onChange={e => setSelectedId(e.target.value)}>
                {packages.map(p => (
            ...
            {selected && <p className="text-sm text-body">{selected.description}</p>}

            {selected?.name === "Custom" && (
              <div>
                <label className="ec-label" htmlFor="upgrade-custom-price">Custom price (GH₵)</label>
                <input id="upgrade-custom-price" type="number" min="1" className="ec-input" placeholder="0.00"
            ...
            <p className="text-xs text-body/70">A new invoice will be generated and the client will be asked to accept a new service agreement.</p>
          </div>
        )}

        <div className="flex gap-3 justify-end pt-2">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium border border-line text-body rounded-lg hover:bg-off transition-colors">Cancel</button>
